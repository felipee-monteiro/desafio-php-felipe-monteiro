...
final class ChamadosCRUDTest extends TestCase
{
    use RefreshDatabase;
    ...
    public function testShouldNotAllowMeToAccessRoutesWithColaboratorRole(array $routeMetadata): void
    {
        ...
        $response = $this->actingAs($user)->{$method}($this->routePrefix . $route === '/' ?: "/{$route}");

        $response->assertMethodNotAllowed();
    }

    #[DataProvider('getInvalidPrioritiesIdsProvider')]
    public function testShouldNotAllowMeToDeleteAChamadoWithInvalidNames(float|int $id): void
    {
        $user = User::factory()->create([
            'role_id' => 2,
        ]);

        $response = $this->actingAs($user)->get($this->routePrefix . "/{$id}");
