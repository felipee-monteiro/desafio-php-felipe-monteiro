...
final class StatusCRUDTest extends TestCase
{
    ...
    public static function getInvalidIdsProvider(): array
    {
        return [
            [0],
            [\PHP_INT_MAX + 12],
    ...
    public function testShouldNotAllowMeToCreateAPriorityWithInvalidNames(string $name): void
    {
        ...
        $response = $this->actingAs($user)->post($this->routePrefix . '/status/manage', [
            'name' => $name,
        ]);
    ...
    public function testShouldAllowMeToDeleteAPriorityWithInvalidNames(): void
    {
        ...
        $response = $this->actingAs($user)->delete($this->routePrefix . '/status/manage/1');
    ...
    public function testShouldNotAllowMeToUpdateAPriorityWithInvalidNames(string $name): void
    {
        ...
        foreach ($methods as $method) {
            $response = $this->actingAs($user)->{$method}($this->routePrefix . '/status/manage/1', [
                'name' => $name,
            ]);
    ...
    public function testShouldNotAllowMeToManageAStatusWithInvalidNames(float|int $id): void
    {
        $user = User::factory()->create([
            'role_id' => 2,
        ]);
        ...
        foreach ($methods as $method) {
            $response = $this->actingAs($user)->{$method}($this->routePrefix . "/status/manage/{$id}", [
                'name' => 'invalid',
            ]);
